...
1. **读取配置**：从 `config.json` 读取博客配置
2. **解析文章**：从 `posts/` 目录读取Markdown文件，解析元数据和内容
3. **渲染模板**：使用HTML模板渲染每个页面
4. **生成静态文件**：将渲染结果输出到 `build/` 目录
